...
is($dri->get_info('charge'),0,'domain_delete get_info(charge)');
...
#===Test simple transfer initiation===================================
...
$rc=$dri->domain_transfer_start('yahoo.com',{auth => {pw => 'someauthinfo'}, do_simple_transfer => 1});
is_string(munge(shift(@R1)),munge($r),'domain_simple_transfer_start');
is($rc->is_success(),1,'domain_simple_transfer_start is_success');
is($dri->get_info('simple_transfer_job_id'),3735288,'domain_simple_transfer_start get_info(id)');

#===Test simple transfer check===================================
...
is_string(munge(shift(@R1)),munge($r),'domain_simple_transfer_query');
is($rc->is_success(),1,'domain_simple_transfer_query is_success');
is($dri->get_info('status'),'pending','domain_simple_transfer_query get_info(status)');
is($dri->get_info('reason'),'Transfer in progress','domain_simple_transfer_query get_info(reason)');
is($dri->get_info('request_date'),'2015-10-13T06:33:44Z','domain_simple_transfer_query get_info(request_date)');
is($dri->get_info('id'),'3735288','domain_simple_transfer_query get_info(id)');
